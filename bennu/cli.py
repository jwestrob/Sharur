...
Example usage:
    bennu overview --db data/bennu.duckdb
    bennu genomes --taxonomy Archaea --db data/bennu.duckdb
    bennu proteins --min-length 2000 --db data/bennu.duckdb
    bennu neighborhood PROTEIN_ID --window 10 --db data/bennu.duckdb
    bennu search --has giant,unannotated --db data/bennu.duckdb
"""
...
def genomes(
    ...
    Examples:
        bennu genomes --taxonomy Archaea
        bennu genomes --min-comp 90 --max-contam 5
    """
    db_path = Path(db)
    if not db_path.exists():
        typer.echo(f"DB not found: {db}", err=True)
...
def proteins(
    ...
    Examples:
        bennu proteins --genome bin_001
        bennu proteins --min-length 2000 --unannotated
    """
    db_path = Path(db)
    if not db_path.exists():
        typer.echo(f"DB not found: {db}", err=True)
...
def neighborhood(
    ...
    Example:
        bennu neighborhood PROTEIN_ID --window 15
    """
    db_path = Path(db)
    if not db_path.exists():
        typer.echo(f"DB not found: {db}", err=True)
...
def search(
    ...
    Examples:
        bennu search --has giant,unannotated
        bennu search --has confident_hit --lacks hypothetical
        bennu search --annotation "hydrogenase"
        bennu search --accession PF00142
    """
    db_path = Path(db)
    if not db_path.exists():
        typer.echo(f"DB not found: {db}", err=True)
...
def compute_predicates(
    ...
    Examples:
        bennu compute-predicates --db data/bennu.duckdb
        bennu compute-predicates --protein PROT_001 --db data/bennu.duckdb
    """
...
def predicates(
    ...
    Use with 'bennu search --has PREDICATE'.
    ...
    Examples:
        bennu predicates
        bennu predicates --category enzyme
        bennu predicates --category transport --hierarchy
    """
    from bennu.predicates.vocabulary import list_predicates, list_categories
