"""Tests for provenance/hypothesis facade methods on Sharur class."""
...
@pytest.fixture
def sharur_instance(tmp_path):
    """Create a Sharur instance with a temporary DuckDB database."""
...
class TestHypothesisRegistryProperty:
    def test_lazy_creation(self, sharur_instance):
        b = sharur_instance
    ...
    def test_path_is_correct(self, sharur_instance):
        b = sharur_instance
...
class TestAddEvidence:
    def test_with_uuid(self, sharur_instance):
        b = sharur_instance
    ...
    def test_with_string_uuid(self, sharur_instance):
        b = sharur_instance
        h = b.propose_hypothesis("Test")
    ...
    def test_unknown_hypothesis_raises(self, sharur_instance):
        b = sharur_instance
...
class TestListAndSummary:
    def test_list_empty(self, sharur_instance):
        b = sharur_instance
        assert b.list_hypotheses() == []

    def test_list_returns_all(self, sharur_instance):
        b = sharur_instance
        b.propose_hypothesis("H1")
        b.propose_hypothesis("H2")
        assert len(b.list_hypotheses()) == 2

    def test_summary_empty(self, sharur_instance):
        b = sharur_instance
        assert b.hypothesis_summary() == "No hypotheses registered."

    def test_summary_with_evidence(self, sharur_instance):
        b = sharur_instance
...
class TestLogProvenance:
    def test_basic(self, sharur_instance):
        b = sharur_instance
    ...
    def test_appears_in_session(self, sharur_instance):
        b = sharur_instance
        b.log_provenance("q1", "r1")
        b.log_provenance("q2", "r2")
        assert len(b.session.get_provenance()) == 2

    def test_parent_chaining_with_uuids(self, sharur_instance):
        b = sharur_instance
    ...
    def test_parent_chaining_with_strings(self, sharur_instance):
        b = sharur_instance
        e1 = b.log_provenance("Step 1", "result 1")
        e2 = b.log_provenance("Step 2", "result 2", parent_ids=[str(e1.entry_id)])
        assert e2.parent_ids == [e1.entry_id]

    def test_no_parent_ids(self, sharur_instance):
        b = sharur_instance
...
class TestManifestHypothesisIntegration:
    def test_empty_manifest_has_hypotheses_section(self, sharur_instance):
        b = sharur_instance
        assert "hypotheses" in b.manifest.data

    def test_resume_includes_hypotheses(self, sharur_instance):
        b = sharur_instance
    ...
    def test_resume_omits_hypotheses_when_none(self, sharur_instance):
        b = sharur_instance
        status = b.resume()
        assert "### Hypotheses" not in status

    def test_manifest_counts_updated(self, sharur_instance):
        b = sharur_instance
        b.propose_hypothesis("H1")
        b.propose_hypothesis("H2")
