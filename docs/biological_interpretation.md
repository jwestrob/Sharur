...
|---|---|---|
| `source='hyddb'` / `hyddb_subgroup` = `nife_group4` or a Group 4 trait (`h2_evolving`, `energy_conserving_hydrogenase`, `ferredoxin_coupled`, `ech_hydrogenase`) | Group 4 catalytic subunits share the Complex1_49kDa superfamily with respiratory **Complex I NuoD**; the Complex I share varies by dataset and lineage. | Read `hydrogenase_classifications` (domain observations, curation reason), then curate by neighborhood. Full protocol: `.claude/skills/hydrogenase.md` |
| `hyddb_subgroup` = `hyddb_needs_curation`, `hyddb_class_conflict`, or `hydrogenase_complex1_review` | The caller flags a missing catalytic domain, a class disagreement, or Complex I-superfamily domains. | Report the exact subgroup label as a provisional nearest-reference assignment; name a hydrogenase class only after neighborhood curation. |
| Any `hyddb_subgroup` functional trait (uptake, sensor, cofactor coupling, bifurcation) | Sharur subgroups come from the nearest HydDB reference; traits follow Søndergaard et al. 2016 Table 1 for characterized subgroups only. | Pair each trait with the exact reference label from `hydrogenase_classifications.reference_label` and treat it as a hypothesis for the protein. |
...
KO meanings come from KEGG itself. Map a BRITE node or a KEGG module to predicates in `sharur/predicates/mappings/data/kegg_brite_predicates.tsv` or `kegg_module_predicates.tsv` (component-level predicates only; keep modules whose components serve other pathways out), or propose a KO pair in `kegg_predicate_proposals.tsv`, then run `sharur setup-kegg --inputs <KEGG dir>/inputs` to rebuild the local map. A proposed pair ships when KEGG's EC numbers, BRITE placement, module membership, the KO's symbols/name, HydDB reference labels (KOs KEGG names as hydrogenases), or Swiss-Prot consensus supports it (`sharur/predicates/mappings/kegg_evidence.py`).
