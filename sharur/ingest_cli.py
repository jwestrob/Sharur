...
def _synthesize_pipeline(raw_dir: Path, data_dir: Path) -> None:
    ...
    fasta_paths = sorted(raw_dir.glob("*.fna")) + sorted(raw_dir.glob("*.fa")) + sorted(
        raw_dir.glob("*.fasta")
    )
    if not fasta_paths:
...
def run(
    ...
    output: Annotated[
        Path,
        typer.Option("--output", "-o", help="Destination DuckDB path, usually DATASET/sharur.duckdb"),
    ...
    skip_quast: Annotated[bool, typer.Option(help="Skip optional Stage 01 (QUAST)")] = False,
    skip_dfast: Annotated[bool, typer.Option(help="Skip optional Stage 02 (DFAST QC)")] = False,
    ...
    skip_gecco: Annotated[bool, typer.Option(help="Skip optional Stage 05a (GECCO BGC)")] = False,
    skip_dbcan: Annotated[bool, typer.Option(help="Skip legacy Stage 05b (dbCAN helper)")] = False,
    ...
    if mode == "fast":
        if dry_run:
            console.print(
                "[yellow]Dry run: fast mode would synthesize stage outputs (no commands to run).[/yellow]"
            )
            return None
    ...
    spec.loader.exec_module(kb)  # type: ignore[union-attr]
